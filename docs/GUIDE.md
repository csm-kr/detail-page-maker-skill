...
판단 게이트를 새 세션에 맡긴다. 대화 맥락이 넘어가지 않으므로 게이트가 자기 것만 본다.

```text
gate G2 --exec             → 팩 조립 → claude -p → check.mjs 재실행 → 통과/재시도(최대 3회)
run --exec                 → 판단 게이트도 멈추지 않고 완주
pack                       → 게이트마다 세션이 받는 크기. 전량의 1/3 이하여야 한다
pack G2                    → G2 세션이 받는 것을 그대로 출력
```

G2 실측: 팩 14,685B(전량의 10%) · 12.2분 · **1회에 통과** · 사람 개입 0회.

사람이 봐야 끝나는 자리는 실패와 구분한다 — 재시도로 풀리지 않는다.

```text
gate G6 --block "컷 30장을 원본 해상도로 봐야 한다"
gate G6 --start            → 보고 나면 다시 시작한다. 막힘이 풀린다
```

**자식이 "했다" 고 해도 통과가 아니다.** 통과 판정은 언제나 부모의 `check.mjs` 다.
로그는 `work/exec/<게이트>-<n>.json` 과 팩 전문 `<게이트>-<n>.prompt.md` 로 남는다.
결정적인 게이트(G0·G9·G10)에는 `--exec` 가 없다 — 판단할 것이 없다.
