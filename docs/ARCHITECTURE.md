...
| `gates.history.json` | 워크스페이스 | 오케스트레이터 | 회차별 통과·결함·소요. **완화 판단의 근거** |
...
| **미디어 예산 12 MB를 GIF가 넘긴다** | 실측: 사진 21.3 MB + GIF 16.4 MB = 38 MB. 사진은 WebP q85로 4–5 MB. GIF는 상위 3개(`g07-pleats` 5.1 · `g05-shade` 3.8 · `g01-hero` 3.4)가 12.3 MB를 먹고 나머지 7개는 4.1 MB — **큰 3개만 팔레트·프레임·영역을 줄이면 예산에 들어간다.** 그래도 넘으면 애니메이션 WebP를 검토하고 예산을 올린다 |
