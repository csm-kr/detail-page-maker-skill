...
$labelFont = New-Object System.Drawing.Font('Arial', 19, [System.Drawing.FontStyle]::Bold, [System.Drawing.GraphicsUnit]::Pixel)
...
try {
  ...
  $graphics.DrawString('D08 MATCHED PAIR CHECK', $titleFont, $titleBrush, $margin, 26)
  ...
  for ($index = 0; $index -lt $cards.Count; $index += 1) {
    ...
    try {
      ...
      $graphics.DrawString($card.Label, $labelFont, $whiteBrush, $x + 18, $y + 551)
    }
    finally {
      $labelBrush.Dispose()
    }
  ...
  $outputPath = Join-Path $evidenceRoot 'd08-rev003-rejected-revised.jpg'
