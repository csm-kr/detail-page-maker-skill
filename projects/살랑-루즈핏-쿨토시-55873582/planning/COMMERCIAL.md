...
- `status`: REV-022_SALES_FIRST_ASSEMBLED
- `planning_phase`: `G4_QA_IN_PROGRESS`
...
- `core_promise`: `조이지 않게 여유롭고, 손등까지 길게.`
...
|---|---|---|---|
| PAIN-01 | `팔에 꽉 붙는 느낌이 싫어요.` | 구매 질문 카드 | 여유 있게 떨어지는 플리츠 |
| PAIN-02 | `손목에서 끝나면 손등이 그대로 드러나요.` | 손등을 가리키는 대화형 카드 | 엄지홀에서 손등으로 이어지는 길이 |
| PAIN-03 | `스포티한 디자인은 일상복에 부담스러워요.` | 옷차림 위 메시지 카드 | 화이트 플리츠의 데일리 인상 |
...
- `publishable`: true
- `allowed_customer_copy`: `시원한 쿨 소재`, `얇고 가벼운 여름용 쿨 소재`
- `visual_policy`: 온도 그래프·막대·꺾은선·열화상 없이 실제 착용 제품 위의 짧은 쿨 스윕
...
- `customer_copy`: `타이트한 밀착 대신, 여유롭게 떨어지는 루즈핏.`
...
- `customer_copy`: `손목에서 끝나는 팔토시는 손등을 그대로 노출합니다.`
...
- `visual_proof`: 원단 매크로와 실제 착용 제품 위의 짧은 쿨 스윕 GIF
...
- `customer_copy`: `티셔츠부터 출근 룩, 원피스까지 자연스럽게.`
