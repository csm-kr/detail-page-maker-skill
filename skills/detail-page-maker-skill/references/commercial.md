...
고객용 문장과 화면 구성은
[`commercial-copy-tone-guide.md`](commercial-copy-tone-guide.md), 주장 직후의
증거 GIF와 시각 효과는
[`commercial-effects-and-claim-proof.md`](commercial-effects-and-claim-proof.md)를
함께 따른다.
...
`pain_points`는 기본적으로 정확히 3개를 사용한다. 각 불편은 사용 상황·핏·스타일
처럼 서로 다른 구매 망설임을 맡고 같은 문제를 다른 문장으로 반복하지 않는다.
문자나 말풍선은 사용할 수 있지만 고객의 실제 후기나 구매 인증으로 위장하지 않는다.
...
- `numeric_basis: not_supplied`이면 온도 하강으로 읽히는 그래프·막대·꺾은선·
  열화상 범례를 금지한다. 실제 제품 위의 짧은 쿨 스윕과 정성적 색 전환만 허용한다.
- 공개 화면에서는 `제조사 확인`, `제조사 주장`, `MANUFACTURER_CLAIM`을 말하지
  않는다. 내부 사실을 고객이 겪는 상황과 변화의 방향으로 번역한다.
...
4. 서로 다른 선택 이유 3~5개를 고객 편익 문장으로 보여 준다.
5. 각 장점 바로 다음에 실제 구조·사용법·규격을 보여 주는 전용 GIF를 둔다.
6. 증거를 페이지 뒤쪽 갤러리에 한꺼번에 다시 모으지 않는다.
...
이미지를 배경으로 쓰는 섹션은 장면이 카피의 상황을 먼저 전달해야 한다. 제품이
가려지지 않는 여백에 제목을 놓고, 필요한 경우 제한된 그라디언트 스크림만 사용한다.
제품 구성은 가장 동일성이 좋은 한 쌍 이미지에 수량·윤곽을 읽게 하는 짧은 FX를
더한다. 마지막 장면은 실제 길이와 착용 범위가 한눈에 보이게 끝낸다.
...
[`korean-copy-typography.md`](korean-copy-typography.md)와
