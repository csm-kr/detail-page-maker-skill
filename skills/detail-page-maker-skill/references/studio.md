...
Studio의 저작 정본은 편집 가능한 HTML section model이고 유일한 디자인 폭은
390 CSS px다. 고객 전달 자산은 같은 section을 폭 780px로 렌더한다.

## 에셋 승인

Studio는 pending 이미지·GIF를 원본과 함께 표시한다. 승인 또는 반려 버튼은
사용자의 명시적 확인을 요구하며 파일 이동, 해시, 원장 기록을 한 동작으로 처리한다.
pending과 필수 미승인이 0개일 때만 일반 HTML 내보내기를 연다.
...
사용자가 보는 저장 동작은 새 revision 이름을 만들지 않는다. `저장`을 명시적으로
누르면 현재 working model과 `output/detail-page.html`을 덮어쓰고 전체 section의
...
사용자 검토와 고객 출력 진입점은 `output/detail-page.html` 하나다. media는
...
2. `POST /api/v1/studio/working/save`
   ...
   - 현재 `output/detail-page.html`을 원자적으로 덮어쓰기 전에 이전 bytes를
     내부 backup에 기록한다.
...
4. `POST /api/v1/studio/capture/complete`
   ...
   - G4Q artifact와 ValidationReceipt가 일치하고 transition이
     `RubricPublishReady`일 때만 `workflow-advance`로 exact
     `G4U_APPROVAL` challenge를 연다.
...
- 승인된 `G5U_APPROVAL`과 exact subject에 묶인 verified
