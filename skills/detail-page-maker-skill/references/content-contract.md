...
- 문제 2개 이상, 해결 장점마다 1개, 사용 과정 1개 이상, 비교 1개 이상,
  **치수 1개 이상**을 배정한다.
- 치수 motion 은 모든 상품에 항상 있다. 크기는 반품 사유 1순위이고 사진만으로는
  판단되지 않는다. 공급처 표기 치수 또는 실측이 있으면 실제 수치를, 없으면
  기준물 비교로 체감 크기를 보여 준다. 근거가 없다고 생략하지 않는다.
