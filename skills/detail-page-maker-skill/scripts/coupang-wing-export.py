...
def save_animated_webp(frames, durations, output_path):
    if not features.check("webp_anim"):
        raise RuntimeError("Pillow does not support animated WebP")
...
def capture_section(section_id):
    ...
    composite = Image.new("RGB", (VIEWPORT_WIDTH, height))
    for offset in range(0, height, VIEWPORT_HEIGHT):
        ...
        crop_top = desired_y - scroll_y
        if (
            section_left < 0
            or section_left + VIEWPORT_WIDTH > viewport.width
            or crop_top < 0
            or crop_top + remaining > viewport.height
        ):
            raise RuntimeError(
        ...
        crop = viewport.crop(
            (
                section_left,
                crop_top,
                section_left + VIEWPORT_WIDTH,
                crop_top + remaining,
            )
        )
        composite.paste(crop, (0, offset))
    return composite
    ...
    try:
        ...
        js(
            """(() => {
              ...
              style.textContent =
                ...
                '#detailPage{margin:0!important;width:780px!important;' +
                'max-width:780px!important;box-shadow:none!important}';
        ...
        for item in catalog:
            ...
            if animation_items:
                ...
                for animation_item in animation_items:
                    animation_item["layout"] = layout_map[
            ...
            output_filename = (
                f"{item['order']:02d}-{safe_slug(section_id)}.webp"
            )
                ...
                with Image.open(output_path) as encoded:
                    ...
                    for frame_index in range(output_frames):
                        ...
                        encoded_durations.append(
                            int(encoded.info.get("duration", 0) or 0)
                        )
                kind = "animated"
        ...
        manifest = {
            ...
            "local_qa": {
                ...
                "all_width_780": all(
                    asset["width"] == VIEWPORT_WIDTH for asset in results
                ),
                "all_under_10mb": all(
