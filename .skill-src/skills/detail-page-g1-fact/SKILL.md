...
- 공급처 페이지를 **다시** 받는다 — `orchestrate capture --url <공급처> --as supplier`
  - 도매꾹은 `dmk-extractor` 로 간다. 썸네일·펼친 상세설명·상세 안 GIF 프레임·후기 표본을 번들로 받고 manifest 해시가 스스로 등록된다
  - 손 캡처에는 차단 판정도 상품번호 대조도 후기 개인정보 마스킹도 없다. 추출기가 없는 호스트에서만 `orchestrate lock --read <캡처> --url <공급처>` 로 등록한다
...
- 공급처 상세가 한 장으로 오면 `capture` 가 `detail/tiles/` 에 잘라 둔다. **조각을 하나도 빼지 않고 연다** — 긴 원본을 그대로 열면 축소본이라 문자가 사라지고, 거기서 읽었다고 적은 문자는 근거가 아니다
