...
- [`claim-evidence-map.json`](claim-evidence-map.json): 공개 카피 9개와 사실·섹션·자산의 연결
- [`publication-approval.json`](publication-approval.json): 현재 v1의 승인 범위와 제외 항목
- [`assets/manifest.json`](assets/manifest.json): 공급처 원본, ImageGen 파생 이미지와 HyperFrames GIF의 출처·해시·역할

사용자가 공급처 URL을 현재 v1의 기준 사실로 지정해 제품명·기능·구조·치수·재질·원산지·수입원은 공개 가능 상태다. 녹 방지·위생·내구·안전·일정한 절삭 결과·세척법·시험·가격·옵션은 근거가 없어 사용하지 않았다.
...
- 오이 얇게 썰기 사용 흐름: [`videos/use-demo-motion`](../../../videos/use-demo-motion)

두 모션은 HyperFrames `check`에서 오류·경고 0을 통과한 4초·15fps 반복 GIF다. 30fps MP4는 QA용으로 각 프로젝트의 `renders/`에 보존한다. 사용 장면은 성능 증거가 아니라 이해를 돕는 연출로 명시했다.

## 최종 QA

[`qa/behance-rubric-report.md`](qa/behance-rubric-report.md)에 Behance 공통 문법, 320~800px 반응형, 접근성, 축소 모션, 편집 상태와 주장 근거 검수를 기록했다.

최종 판정은 **94/100, 하드 실패 0개**다.
